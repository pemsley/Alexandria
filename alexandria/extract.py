...
# Publisher download filenames encode the article's identifier —
# usually the DOI with its punctuation removed. Worth trusting over
# the PDF's own Info dict: the filename is generated by the
# publisher's download pipeline for *this* file, whereas embedded
# metadata is whatever template the operator left behind. Real case:
# advances-in-direct-methods-…-S0959440X99000202-main.pdf carries
# Title "List of Referees 1999" and a Process Safety and
# Environmental Protection DOI in its Info dict.

# Supporting-information files decode to their *parent* article's
...
_PII_RE = re.compile(r"S(\w{4})(\w{4})(\d{2})(\d{5})(\w)",
                     re.IGNORECASE)

_FILENAME_DOI_RULES = (
    # (compiled pattern, formatter)
    # s41467 (Nature portfolio), s13321 (BMC) and s00775 (Springer)
    # share a shape but not a prefix — measured against the library,
    # a single 10.1038 rule mislabelled the Springer ones.
    (re.compile(r"^(s4\d{4}-\d{3}-\d{4,5}-[a-z0-9])$", re.I),
     lambda m: "10.1038/" + m.group(1)),
    (re.compile(r"^(s1\d{4}-\d{3}-\d{4,5}-[a-z0-9])$", re.I),
     lambda m: "10.1186/" + m.group(1)),
    (re.compile(r"^(s0\d{4}-\d{3}-\d{4,5}-[a-z0-9])$", re.I),
     lambda m: "10.1007/" + m.group(1)),
    (re.compile(r"^(d\d[a-z]{2}\d{5}[a-z])$", re.I),
     lambda m: "10.1039/" + m.group(1)),
    (re.compile(r"^(acs\.[a-z0-9]+\.\w+)$", re.I),
     lambda m: "10.1021/" + m.group(1)),
    (re.compile(r"^(pnas\.\d{7,})$", re.I),
     lambda m: "10.1073/" + m.group(1)),
    (re.compile(r"^((?:science|sciadv|scisignal|scitranslmed)\.\w+)$",
                re.I),
     lambda m: "10.1126/" + m.group(1)),
    # bioRxiv / medRxiv: 2024.04.29.591684[v1][.full]
    (re.compile(r"^(\d{4}\.\d{2}\.\d{2}\.\d{6})(?:v\d+)?$", re.I),
     lambda m: "10.1101/" + m.group(1)),
    # arXiv: 2006.11239[v2]
...
def doi_from_filename(path):
    """Derive the DOI a publisher encoded in a download filename, or
    None. Lower-cased, so callers can compare with normalised DOIs.

    Refuses supporting-information files: they decode to the parent
    article's DOI, and labelling a supplement as its parent paper
    would be worse than leaving it unidentified."""
    if not path:
        return None
    name = os.path.basename(str(path))
    stem = re.sub(r"\.pdf$", "", name, flags=re.IGNORECASE)
    if _SI_MARKER_RE.search(stem):
        return None
    # Elsevier: the PII appears inside a longer name
    # ("1-s2.0-<PII>-main", "…-<PII>-molecular-recognition…").
    if re.search(r"1-s2\.0-", stem, re.IGNORECASE):
        m = _PII_RE.search(stem)
        if m:
            return "10.1016/s{}-{}({}){}-{}".format(
                m.group(1).lower(), m.group(2).lower(),
                m.group(3), m.group(4), m.group(5).lower())
    # Everything else keys off the whole stem, after dropping the
    # duplicate-download suffix ("-1") and the ".full" of a bioRxiv
    # download.
    cleaned = re.sub(r"-\d+$", "", stem)
    cleaned = re.sub(r"\.full$", "", cleaned, flags=re.IGNORECASE)
    for pattern, fmt in _FILENAME_DOI_RULES:
